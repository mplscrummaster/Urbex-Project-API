...
try {
  ...
  if (args.has("--publish")) {
    const info = db
      .prepare(
        "UPDATE scenarios SET is_published=1 WHERE _id_scenario IN (SELECT _id_scenario FROM scenarios ORDER BY _id_scenario LIMIT 5) AND is_published=0"
      )
      .run();
  ...
  if (args.has("--media")) {
    ...
    const insertBlock = db.prepare(
      `INSERT INTO blocks (owner_type,_id_mission,position_block,type_block,content_text,url_media,caption) VALUES ('mission',?, 99,'image',NULL,?,?)`
    );
    let added = 0;
