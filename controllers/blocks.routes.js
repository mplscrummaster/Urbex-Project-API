...
function buildScenarioBlockCreator(ownerType) {
  return function scenarioBlockCreateHandler(req, res) {
    ...
    try {
      ...
      if (!Number.isFinite(position)) {
        const maxPos = db
          ...
          .get(ownerType, scenarioId)?.maxp;
        position = Number(maxPos) + 1;
      }
      const created = insertScenarioBlock({
        ownerType,
        scenarioId,
        position,
        type: type_block,
...
function buildScenarioBlockReorder(ownerType) {
  return function scenarioBlockReorderHandler(req, res) {
    const scenarioId = Number.parseInt(req.params.id, 10);
    if (!Number.isFinite(scenarioId) || scenarioId <= 0)
      return res.status(400).json({ error: "invalid scenario id" });
    ...
    if (items.length === 0)
      return res
        .status(400)
        .json({ error: "expected array of {id, position}" });
    try {
      if (!canEditScenario(req.auth?.sub, scenarioId))
        return res.status(403).json({ error: "forbidden" });
      const trx = db.transaction((updates) => {
        for (const u of updates) {
          ...
          ).run(u.position, u.id, ownerType, scenarioId);
        }
      });
      trx(items);
      const rows = db
...
// Scenario intro/outro creation using helpers
router.post(
  "/scenarios/:id/intro/blocks",
  requireAuth,
  buildScenarioBlockCreator("scenario_intro")
);
router.post(
  "/scenarios/:id/outro/blocks",
  requireAuth,
  buildScenarioBlockCreator("scenario_outro")
);
...
// REORDER scenario intro/outro using helper
router.put(
  "/scenarios/:id/intro/blocks/reorder",
  requireAuth,
  buildScenarioBlockReorder("scenario_intro")
);
router.put(
  "/scenarios/:id/outro/blocks/reorder",
  requireAuth,
  buildScenarioBlockReorder("scenario_outro")
);
