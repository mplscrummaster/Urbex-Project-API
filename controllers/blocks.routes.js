...
router.post("/scenarios/:id/intro/blocks", requireAuth, (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(position)) {
      const maxPos = db
        .prepare(
          `SELECT COALESCE(MAX(position_block), 0) AS maxp FROM blocks WHERE owner_type = 'scenario_intro' AND _id_scenario = ?`
        )
        .get(scenarioId)?.maxp;
    ...
    const info = db
      .prepare(
        `INSERT INTO blocks (owner_type, _id_scenario, position_block, type_block, content_text, url_media, caption)
         VALUES ('scenario_intro', ?, ?, ?, ?, ?, ?)`
      )
      .run(scenarioId, position, type_block, content_text, url_media, caption);
    const created = db
      .prepare(
...
// UPDATE scenario intro blocks order
...
// UPDATE scenario mission blocks order
...
router.post("/scenarios/:id/outro/blocks", requireAuth, (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(position)) {
      const maxPos = db
        .prepare(
          `SELECT COALESCE(MAX(position_block), 0) AS maxp FROM blocks WHERE owner_type = 'scenario_outro' AND _id_scenario = ?`
        )
        .get(scenarioId)?.maxp;
      position = Number(maxPos) + 1;
    }
...
// UPDATE scenario outro blocks order
