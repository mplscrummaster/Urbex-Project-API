...
router.get('/scenarios/:id/fullVue', (req, res) => {
  ...
  try {
    ...
    const intro_blocks = parseJSON(row.intro_blocks, []);
    const missions = parseJSON(row.missions, []);
    const outro_blocks = parseJSON(row.outro_blocks, []);
    ...
    return res.json({
      _id_scenario: row._id_scenario,
      title_scenario: row.title_scenario,
